...
Só segue sem perguntar o que já está **explícito** na instrução do usuário ou **fechado** em `docs/` / rules (ex.: nomenclatura `list/get/create`, parâmetro `customerSchema`, envelope `{ data }`).
...
1. Classificar módulo (`customers`, `scheduling`, …), entidade, operação (`list|get|create|update|delete` ou verbo de domínio).
...
- Arquivo `snake_case`: `customer_create.service.ts`.
- Parâmetro: `customerSchema` — **nunca** `data`.
- Update: `execute(ctx, customerId, customerSchema)`.
